  ...
  generateReport(profile) {
    return {
      ...
      summary: {
        ...
        longestMeasure: profile.measures.reduce(
          (max, measure) => (measure.duration > max.duration ? measure : max),
          { duration: 0 }
        ),
      },
    };
...
class MemoryAnalyzer {
  ...
  compareSnapshots(snapshot1Id, snapshot2Id) {
    ...
    const comparison = {
      ...
      totalJSHeapSizeDiff:
        snapshot2.totalJSHeapSize - snapshot1.totalJSHeapSize,
      timeDiff: snapshot2.timestamp - snapshot1.timestamp,
    };

    console.group(
      `Memory Comparison: ${snapshot1.label} vs ${snapshot2.label}`
    );
    console.log(
      'Used Heap Size Change:',
      this.formatBytes(comparison.usedJSHeapSizeDiff)
    );
    console.log(
      'Total Heap Size Change:',
      this.formatBytes(comparison.totalJSHeapSizeDiff)
    );
  ...
  startMemoryMonitoring(interval = 5000) {
    const monitor = setInterval(() => {
      ...
      if (memoryInfo) {
        ...
        console.log(
          `Memory Usage: ${this.formatBytes(memoryInfo.usedJSHeapSize)} (${usagePercent.toFixed(2)}%)`
        );
      }
    }, interval);
  ...
  // 检测内存泄漏
  detectMemoryLeaks(threshold = 10 * 1024 * 1024) {
    // 10MB
    ...
    if (memoryIncrease > threshold) {
      console.warn(
        `Potential memory leak detected: ${this.formatBytes(memoryIncrease)} increase`
      );
      return true;
    }
...
class DOMDebugger {
  ...
  highlight(selector, color = 'red') {
    ...
    console.log(
      `Highlighted ${elements.length} elements matching "${selector}"`
    );
    return elements;
  }
    ...
    window.XMLHttpRequest = function () {
      ...
      xhr.send = function (body) {
        ...
        xhr.addEventListener('loadend', () => {
          ...
          request.responseHeaders = self.parseResponseHeaders(
