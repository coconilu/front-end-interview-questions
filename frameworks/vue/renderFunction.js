  ...
  render(h) {
    ...
    return h(
      ...
      this.items.map((item, index) =>
        h('li', { key: item.id }, [h('span', `${index + 1}. ${item.name}`)])
      )
    );
  },
};
...
const FunctionalComponent = {
  ...
  render(h, context) {
    ...
    return h('div', { class: 'heading-wrapper', ...data }, [
      h(tag, props.title),
    ]);
  },
};
...
const JSXComponent = {
  ...
  render() {
    ...
    return (
      <div
        class={{ wrapper: true, active: this.isActive }}
  ...
  render() {
    return (
      <div>
        ...
        {/* 使用插槽 */}
        <div class="footer">{this.$slots.footer || <p>Default Footer</p>}</div>
      </div>
    );
  },
};
