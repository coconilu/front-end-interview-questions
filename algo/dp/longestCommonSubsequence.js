...
function test() {
    ...
    for (const { text1, text2, expected, expectedLCS } of testCases) {
        ...
        console.log(`LCS长度: ${length}, 期望: ${expected}, 正确: ${length === expected}`);
        console.log(`优化空间后LCS长度: ${optimizedLength}, 期望: ${expected}, 正确: ${optimizedLength === expected}`);
        console.log(`LCS内容: "${lcs}", 期望: "${expectedLCS}", 正确: ${lcs === expectedLCS}`);
        console.log('---');
    }
