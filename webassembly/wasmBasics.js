...
class WasmMath {
  ...
  async matrixMultiply(matrixA, matrixB, rowsA, colsA, colsB) {
    ...
    try {
      ...
      // 调用 WASM 函数
      this.interop.callFunction('matrix_multiply', 
        offsetA, offsetB, offsetC, rowsA, colsA, colsB);
...
class WasmImageProcessor {
  ...
  async resizeImage(imageData, newWidth, newHeight) {
    ...
    try {
      ...
      this.interop.callFunction('resize_image',
...
class WasmBenchmark {
  ...
  async mathBenchmark(wasmMath) {
    ...
    // WASM 版本
    const wasmSum = () => {
      return wasmMath.interop.callFunction('array_sum', 
        /* 需要先将数组写入内存 */);
    };
  ...
  generateReport() {
    ...
    const avgSpeedup = this.results.reduce((sum, result) => 
      sum + parseFloat(result.speedup), 0) / this.results.length;
