...
class ComponentFactory {
  ...
  // 批量创建组件
  createBatch(configs) {
    return configs.map((config) => this.create(config.type, config.props));
  }
}
...
class EventEmitter {
  ...
  on(event, listener) {
    ...
    if (listeners.length >= this.maxListeners) {
      console.warn(
        `MaxListenersExceededWarning: Possible EventEmitter memory leak detected. ${listeners.length + 1} ${event} listeners added.`
      );
    }
